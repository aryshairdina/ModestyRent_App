...
public class activity_product_details_onboarding extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        calendarAvailability.setOnDateChangeListener((view, year, month, dayOfMonth) -> {
            ...
            if (!waitingForEnd) {
                ...
                Toast.makeText(activity_product_details_onboarding.this, "Start date selected. Now tap end date.", Toast.LENGTH_SHORT).show();
            } else {
                // choose end
        ...
        // --------------------------
        // IMPORTANT: only change below for btnBook & btnChatOwner
        // Both will now open activity_signin (as requested).
        // Calendar and other behavior remain unchanged.
        // --------------------------

        btnBook.setOnClickListener(v -> {
            // Redirect user to sign-in page before booking
            Intent i = new Intent(activity_product_details_onboarding.this, activity_signin.class);
            startActivity(i);
        });

        btnChatOwner.setOnClickListener(v -> {
            // Redirect user to sign-in page before chatting
            Intent i = new Intent(activity_product_details_onboarding.this, activity_signin.class);
            startActivity(i);
        });
    ...
    private void loadProduct() {
        productsRef.get().addOnCompleteListener(task -> {
            if (task.isSuccessful() && task.getResult().exists()) {
                ...
                Toast.makeText(activity_product_details_onboarding.this, "Failed to load product", Toast.LENGTH_SHORT).show();
            }
        });
    }
